...
const Id = () => {
  ...
  const {data, isFetching} = useCourseByIdByAdmin({courseId: id, onError});
    ...
    <>
      <FormLayout>
        <div className={"flex items-center justify-between border-b border-gray-300 py-4 mb-8"}>
          ...
          {/*<div className={"flex items-center gap-2 bg-green-300 rounded-2xl text-sm text-green-500 font-medium py-1 px-3"}>*/}
            ...
            <>
              {isFetching ? (
                ...
                <>
                  ...
                  <div className={"flex flex-col md:flex-row gap-2 md:gap-4 mb-4"}>
                    <div className={"shrink-0 md:w-[200px] text-sm font-medium text-dark-500"}>Теги</div>
                    <div className={"grow flex flex-wrap gap-2"}>
                      {data?.data?.tags && data?.data?.tags.length ? (
                        <>
                          {data?.data?.tags.map((item: string) =>
                            <Tag key={item} title={item} className={"whitespace-nowrap !text-xs"}/>
                          )}
                        </>
                      ) : null}
                    </div>
                  ...
                  <div className={"flex flex-col md:flex-row gap-2 md:gap-4 mb-4"}>
                    ...
                    <div className={"grow"}>{data?.data?.description}</div>
                  </div>
                    ...
                    <div className={"grow"}>
                      {data?.data?.image ? (
                        <div className={"w-full md:w-[480px] h-[320px] rounded-lg border border-gray-200 p-4"}>
                          <img src={data?.data?.image} alt={""} className={"w-full h-full object-contain"}/>
                        </div>
                      ) : null}
                    </div>
                  </div>
                  {data?.data?.lessons && data?.data?.lessons.length ? (
                    <>
                      {data?.data?.lessons.map((item: any, idx: number) => (
                        <div key={idx} className={"mb-10"}>
                          <p className={"text-xl font-medium mb-4"}>Урок {idx + 1}</p>
                          <div className={"flex flex-col md:flex-row gap-2 md:gap-4 mb-4"}>
                            <div className={"shrink-0 md:w-[200px] text-sm font-medium text-dark-500"}>Наименование</div>
                            <div className={"grow"}>{item?.title}</div>
                          </div>
                          <div className={"flex flex-col md:flex-row gap-2 md:gap-4 mb-4"}>
                            <div className={"shrink-0 md:w-[200px] text-sm font-medium text-dark-500"}>Ссылка на видео</div>
                            <div className={"grow"}>{item?.videoUrl}</div>
                          </div>
                          <div className={"flex flex-col md:flex-row gap-2 md:gap-4"}>
                            <div className={"shrink-0 md:w-[200px] text-sm font-medium text-dark-500"}>Полное описание</div>
                            <div className={"grow"}>{item?.bodyText}</div>
                          </div>
                        </div>
                      ))}
                    </>
